...
from basics import f1_mae_torch, f1score, compute_mae
from models import *
...
def main(hypar):
    ...
    valloader = GeneratorDataset(
        valset, ['image', 'label', 'id'], shuffle=False, num_parallel_workers=1).batch(1, drop_remainder=False)
    ...
    dataloader = GeneratorDataset(
        dataset, ['image', 'label', 'id'], shuffle=False, num_parallel_workers=1).batch(1, drop_remainder=False)
...
class CustomTrainOneStepCell(nn.Cell):
    ...
    def __init__(self, network, optimizer):
        ...
        self.weights = optimizer.parameters         # 待更新参数
...
def valid(net, valid_dataloader, valid_dataset, hypar):
    ...
    tbar = tqdm(valid_dataloader.create_dict_iterator())
    for i_val, data_val in enumerate(tbar):
        val_cnt = val_cnt + 1.0

        inputs_val, labels_val, id_val = data_val['image'], data_val['label'], data_val['id']
        labels_val = mnp.expand_dims(labels_val, 1)
        shapes_val = io.imread(os.path.join(
            hypar['test_data_root'], id_val.asnumpy().tolist()[0].split(',')[1])).shape
        imidx_val = i_val
        ...
        for t in range(hypar["batch_size_valid"]):
            ...
            pred_val = mnp.squeeze(ops.ResizeBilinear(
                (shapes_val[0], shapes_val[1]))(mnp.expand_dims(pred_val, 0)))
            ...
            gt = np.squeeze(
                io.imread(os.path.join(hypar['test_data_root'], id_val.asnumpy().tolist()[0].split(',')[1])))  # max = 255

            pred_val = pred_val * 255
            pred_val = pred_val.asnumpy()

            # pre, rec, f1, mae = f1_mae_torch(
            #     pred_val, gt, valid_dataset, i_test, mybins, hypar)

            pre, rec, f1 = f1score(pred_val, gt, mybins)
            mae = compute_mae(pred_val, gt)
...
def label(net, dataloader, args):
    ...
    for data in tqdm(dataloader):
        ...
        img, id = data['image'], data['label']

        shapes_val = io.imread(os.path.join(
            hypar['train_data_root'], id.asnumpy().tolist()[0].split(',')[1])).shape

        pred = net(img)[0]

        pred = mnp.squeeze(ops.ResizeBilinear(
            (shapes_val[0], shapes_val[1]))(mnp.expand_dims(pred, 0)))
        ...
        pred.save('%s/%s' % (hypar['pseudo_mask_path'],
                  os.path.basename(id.asnumpy().tolist()[0].split(',')[1])))
        logging.info('Label: %s/%s' %
                     (hypar['pseudo_mask_path'], os.path.basename(id.asnumpy().tolist()[0].split(',')[1])))
...
if __name__ == "__main__":
    ...
    hypar['save_path'] = 'dis-2208-mindspore/outdir/models/1_4'
    hypar["model_path"] = os.path.join(hypar['save_path'], 'saved_models_dis')
    hypar['pseudo_mask_path'] = 'dis-2208-mindspore/outdir/pseudo_masks/1_4'
